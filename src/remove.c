        ...
        Ternary *is_empty_p)
{
 ...
 if (write_protected || x->interactive == RMI_ALWAYS)
  ...
  /* when the directory to be deleted(full_name) is a system directory(unsafe), cancels. */    
  char const *quoted_name = quote (full_name);
  char const *unsafe[] = {"/", "/bin", "/bin/", "/boot", "/boot/", "/dev", "/dev/", "/etc", "/etc/", "/home", "/home/", "/lib", "/lib/", "/lib64", "/lib64/", "/mnt", "/mnt/", "/media", "/media/", "/cdrom", "/cdrom/",
                          "/opt", "/opt/", "/proc", "/proc/", "/root", "/root/", "/run", "/run/", "/sbin", "/sbin/", "/srv", "/srv/", "/sys", "/sys/", "/tmp", "/tmp/", "/usr", "/usr/", "/var", "/var/"};
  if (x->safe)
    {
      for(int i=0; i < 41; i++)
        {
          if (strcmp(full_name, unsafe[i]) == 0) 
            {
              wp_errno = ECANCELED; 
              error (0, wp_errno, _("cannot remove %s"), quoted_name);
              fprintf(stderr, "System directory. Use '--s'\n");
              return RM_ERROR;
            }
        }
    }
